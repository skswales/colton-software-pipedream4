    ...
    _InoutRef_  P_SS_DATA p_ss_data)
{
    if(RPN_TMP_ARRAY == ss_data_get_data_id(p_ss_data))
    {
        ...
        p_ss_data->arg.ss_array.y_size *= p_ss_data->arg.ss_array.x_size;
        p_ss_data->arg.ss_array.x_size = 1;
    }
}
...
PROC_EXEC_PROTO(c_median)
{
    ...
    for(;;)
    {
        ...
        if(ss_data_temp_array.data_id == DATA_ID_ERROR)
        {
        ...
        ss_data_array_flatten(&ss_data_temp_array); /* SKS 04jun96 - flatten so we can median() on horizontal and rectangular ranges */
        ...
        {
        SS_DATA ss_data;
        (void) array_range_index(&ss_data, &ss_data_temp_array, 0, y_half, EM_REA);
        ...
        if(0 == (y_size & 1))
        {
            ...
            SS_DATA ss_data;
            (void) array_range_index(&ss_data, &ss_data_temp_array, 0, y_half - 1, EM_REA);
            median = (median + ss_data_get_real(&ss_data)) / 2.0;
        }

        ss_data_set_real_try_integer(p_ss_data_res, median);
...
/* performed by ARRAY_RANGE_MEDIAN in Fireworkz */
