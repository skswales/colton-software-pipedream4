...
#if defined(HTML_COLOUR_NAME_TABLE)
...
{
    struct _html_color_name_packed {
    unsigned int rgb : 24;
    unsigned int name_length : 8;
    } packed;
    const char * name;
}
html_color_name[] =
...
PROC_BSEARCH_PROTO(static, html_color_name_lookcomp, struct HTML_COLOR_NAME_BSEARCH_KEY *, struct HTML_COLOR_NAME)
{
    ...
    // reportf("k:%d(%s) d:%d/%d(%s)", key->name_length, key->name, datum->packed.name_length, strlen(datum->name), datum->name);

    /* NB no current_p_docu global register furtling required */

    /* compare leading substrings */
    if(0 != (res = C_strnicmp(key->name, datum->name, MIN(key->name_length, datum->packed.name_length))))
        return(res);

    return((int) key->name_length - (int) datum->packed.name_length);
}

#else

/* lookup using messages file */
...
static char * colmap_messages___block;

static void
colmap_messages_init(void)
{
    if(NULL != colmap_messages___block) return;

    char filename[256];
    int tmp_length = res_findname("ColMapHTML", filename);
    if(tmp_length > 0)
        messages_readfile(&colmap_messages___block, filename);
}

static inline const char *
colmap_messages_lookup(const char * tag_and_default)
{
    colmap_messages_init();

    return(messages_lookup(&colmap_messages___block, tag_and_default, TRUE /*->lookup is case-insensitive*/));
}

#endif /* HTML_COLOUR_NAME_TABLE */
...
{
    const PC_USTR spos = pos;
    PC_USTR epos;
    U32 name_length;
    U32 hex_value;
    U32 r, g, b;

    // already done *p_r = *p_g = *p_b = 0;

    epos = strchr(spos, text_at_char);
    assert(NULL != epos);
    if(NULL == epos)
        return(0);
    ...
    if(NULL == p)
        return(0);
    hex_value = p->packed.rgb;
#else
    U8Z buffer[32];
    if(name_length >= elemof32(buffer))
        return(0);
    short_memcpy32nz(buffer, spos, name_length);
    buffer[name_length] = CH_NULL;
    PC_USTR hex_pos = colmap_messages_lookup(buffer);
    if(NULL == hex_pos)
        return(0);
    if('#' != *hex_pos++)
        return(0);
    char * end_hex_pos;
    hex_value = (U32) strtoul(hex_pos, &end_hex_pos, 16);
    switch(end_hex_pos - hex_pos)
    {
#if CHECKING
        case 0:
#endif
